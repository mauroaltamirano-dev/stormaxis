...
export function Register() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', width: '100%', alignSelf: 'stretch', position: 'relative', overflow: 'hidden', backgroundColor: '#05070A' }}>
      {/* Background */}
      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'url(/images/1132707.jpg)', backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.35, filter: 'grayscale(80%)', mixBlendMode: 'luminosity' }} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: 'clamp(2.5rem, 6vw, 5rem)', position: 'relative', zIndex: 1, minWidth: 0 }}>
        ...
        <div style={{ maxWidth: '520px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.75rem', border: '1px solid rgba(124,77,255,0.4)', background: 'rgba(124,77,255,0.08)', padding: '0.4rem 1rem', marginBottom: '2rem' }}>
            <span style={{ width: '6px', height: '6px', background: '#7c4dff', display: 'block' }} />
            <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.3em', color: '#7c4dff' }}>
              Nuevo Operativo
            </span>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {[
              ...
              { label: 'Draft y veto de mapas por capitanes', color: '#7c4dff' },
              { label: '3 regiones activas', color: '#f0a500' },
            ].map(f => (
              <div key={f.label} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', fontSize: '0.85rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#e2e8f0' }}>
                <div style={{ width: '6px', height: '6px', background: f.color, flexShrink: 0 }} />
                {f.label}
              </div>
            ))}
          </div>
      ...
      {/* Right panel — form */}
      <div style={{ width: 'clamp(380px, 38vw, 560px)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 1, borderLeft: '1px solid rgba(255,255,255,0.06)', background: 'rgba(5,7,10,0.88)', backdropFilter: 'blur(16px)', padding: 'clamp(2.5rem, 5vw, 4rem)', overflowY: 'auto' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: 'linear-gradient(90deg, transparent, #7c4dff 50%, transparent)' }} />

        <div style={{ width: '100%', maxWidth: '420px', paddingTop: '1rem', paddingBottom: '1rem' }}>
          ...
          {/* OAuth */}
          <a href="/api/auth/discord" style={{ textDecoration: 'none', display: 'block', marginBottom: '1.5rem' }}>
            <button style={{ ...oauthBtn('#5865F2'), border: '1px solid rgba(88,101,242,0.5)' }}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <Field label="Nombre de usuario" error={errors.username?.message}>
              <input {...register('username')} placeholder="ZeroX" style={inputStyle} />
            </Field>
            ...
            <button type="submit" disabled={isSubmitting} style={{
              ...
              {isSubmitting ? 'Creando...' : <><span>Crear Cuenta</span><ChevronRight size={18} /></>}
            </button>
          </form>

          <p style={{ textAlign: 'center', marginTop: '2rem', fontSize: '0.8rem', color: '#475569' }}>
            ¿Ya tenés cuenta?{' '}
            <Link to="/login" style={{ color: '#00c8ff', textDecoration: 'none', fontWeight: 700 }}>
              Iniciá sesión
            </Link>
          </p>
        </div>
